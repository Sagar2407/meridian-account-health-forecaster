...
| D-048 | Keep the frozen bands despite a 2.9% auto-release rate | **Superseded by D-061** | Held while the only question on the table was how much unreviewed error to buy. It stopped being the question once the confidence scale itself was measured as under-confident ([Phase 10](PHASE_10_STATUS.md)) |
...
| D-059 | Bake the runtime data slice into the production image rather than mounting it or building it | Accepted | Measurement changed the answer: what the served application reads is 51 MB, not the whole archive, because `metadata.sqlite` already stores every document's text. The original wording here -- "the 28 MB `rag_corpus` is only needed to build the index" -- was wrong, and the deployment proved it: `rag_corpus/knowledge_base.jsonl` is read on every search, because `load_verified_index` rebuilds the parent documents to check the index digest. That one 47 KB file ships; the rest of `rag_corpus` genuinely does not. The image grew 1.47 → 1.55 GB, the free plan needs no disk, and `data/app` landing in the container layer closes O-006 as a side effect ([`docs/DEPLOYMENT.md`](DEPLOYMENT.md)) |
...
| D-061 | Move the green band from 0.85 to 0.80, and the two band-relative caps from 0.84 to 0.79 with it | Accepted | The bands were not the defect; the scale under them was. Composed confidence sits about 0.19 below observed accuracy on development (mean 0.7398 against 0.9270 accuracy, every reliability bin below the diagonal), so 38 development runs scored 0.80-0.90 while being right every time, under a band starting at 0.85. Moving green alone would have been unsafe: section 16.1's caps are defined one hundredth below the band they hold a run under, so leaving `cap_exhausted_retrieval_gap` at 0.84 would have auto-released four runs with an exhausted retrieval gap -- the precise outcome the cap exists to prevent. `__post_init__` now refuses any set where a cap is not below its band. Chosen on development data only per section 22.7, then measured once: development auto-release 6 to 15 of 207 with 0 errors, held out 0 to 2 of 53 with 0 errors and neither capped. Macro F1 and ECE are unchanged, because a threshold routes a forecast rather than making it |
| D-062 | Count only green as auto-released in the Tree-of-Thought ablation, matching the serving path | Accepted | `meridian_eval.tot_ablation` defined `AUTO_RELEASED_ROUTES` as `{"green", "amber"}` while `meridian.serving.scan` defined the same name as `{"green"}`. Section 16.5 puts amber in asynchronous review, so an amber answer is reviewed and counting it overstated what reaches a user unchecked: the ablation reported 40 and 17 auto-released where the system's own definition gives 9 and 6, and escalation rates of 0.623 and 0.840 where they are 0.915 and 0.943. The existing test asserted only that green was in the set and red was not, which held for both definitions; it now asserts the two constants are equal ([Phase 6](PHASE_6_STATUS.md)) |
...
| P-005 | Green ≥0.85, amber 0.70–0.84, red <0.70 | Implemented with every section 16.5 condition ([Phase 5](PHASE_5_STATUS.md)), then **changed in thresholds v2 to green ≥0.80, amber 0.70–0.79** on development evidence (D-061). The plan's numbers assumed a confidence scale the system does not produce |
...
| ~~O-005~~ | ~~Whether to loosen the release bands, given that auto-release is near zero~~ | **Closed: green moved 0.85 to 0.80 in thresholds v2, see D-061** |
